...
(function (ExtendedStatus) {
    ExtendedStatus[ExtendedStatus["RESUMING"] = 15] = "RESUMING";
...
export default class XmppConnection extends Listenable {
    ...
    */
     ...
     * @returns {void}
     */
    _maybeStartWSKeepAlive(forcedTimeout) {
        ...
        if (this._usesWebsocket && websocketKeepAlive > 0) {
            ...
            this._wsKeepAlive = setTimeout(() => this._keepAliveAndCheckShard()
                .then(() => this._maybeStartWSKeepAlive()), interval);
        }
    }
    /**
     ...
     */
    _tryResumingConnection(force = false) {
        const { streamManagement } = this._stropheConn;
     ...
     */
    refreshToken(serviceUrl) {
        this._stropheConn.service = serviceUrl;
        return new Promise((resolve, reject) => {
            let timeoutId = undefined;
            const unsubscribe = this.addCancellableListener(XmppConnection.Events.CONN_STATUS_CHANGED, status => {
