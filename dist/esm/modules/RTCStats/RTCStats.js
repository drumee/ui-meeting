...
class RTCStats {
    ...
    startWithConnection(connection) {
        ...
        const { analytics: { rtcstatsEndpoint: endpoint = '', rtcstatsEnabled = false, rtcstatsPollInterval: pollInterval = 10000, rtcstatsSendSdp: sendSdp = false } = {}, } = options;
        ...
        if (!this._initialized) {
            rtcstatsInit({ statsEntry: this.sendStatsEntry.bind(this) }, { eventCallback: event => this.events.emit(RTCStatsEvents.RTC_STATS_PC_EVENT, event),
                pollInterval,
                sendSdp,
                useLegacy: false });
    ...
    attachToConference(conference) {
        ...
        conference.once(JitsiConferenceEvents.CONFERENCE_CREATED_TIMESTAMP, (timestamp) => {
            this.sendStatsEntry(RTCStatsEvents.CONFERENCE_START_TIMESTAMP_EVENT, null, timestamp);
        });
        conference.once(JitsiConferenceEvents.BEFORE_STATISTICS_DISPOSED, () => this._defaultLogCollector?.flush());
    }
    /**
     ...
     */
    getDefaultLogCollector(maxEntryLength = 10000) {
