...
export default class ConnectionQuality {
    ...
    constructor(conference, eventEmitter, options) {
        ...
        conference.on(ConferenceEvents.BRIDGE_BWE_STATS_RECEIVED, bwe => {
    ...
    _calculateConnectionQuality(videoType, isMuted, resolutionName) {
        ...
        if (isMuted || !resolution || videoType === VideoType.DESKTOP
            ...
            if (activeTPC) {
                ...
                let target = getTarget(activeTPC.doesTrueSimulcast(undefined), resolution, millisSinceStart, activeTPC.getTargetVideoBitrates(undefined));
    ...
    _updateLocalStats(tpc, data) {
        // Update jvbRTT
        if (!tpc.isP2P) {
            const jvbRTT = data.transport?.length && data.transport[0].rtt;
        ...
        const resolutionName = localVideoTrack
            ? Math.min(localVideoTrack.resolution, localVideoTrack.maxEnabledResolution).toString() : null;
        if (!isMuted) {
            this._maybeUpdateUnmuteTime();
        }
        ...
        if (updateLocalConnectionQuality) {
            this._updateLocalConnectionQuality(this._calculateConnectionQuality(videoType, isMuted, resolutionName));
        }
        this.eventEmitter.emit(ConnectionQualityEvents.LOCAL_STATS_UPDATED, this._localStats);
        this._broadcastLocalStats();
    }
